    ...
    <main className="max-w-7xl mx-auto px-6 py-12 space-y-24">
      ...
      {easyRecipes.length > 0 && (
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            <h2 className="text-3xl font-serif font-bold text-gray-900">Легкий старт</h2>
      ...
      <section className="space-y-10">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-serif">Вибір гурманів</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {topRated.map((recipe) => (
            <div key={recipe.id} className="relative group">
              <div className="absolute -top-4 -left-4 w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center text-xl z-10 shadow-lg font-serif italic">
                #
              </div>
              <RecipeCard recipe={recipe} />
            </div>
      ...
      <section className="space-y-12">
        <div className="flex justify-between items-end border-b-2 border-gray-100 pb-6">
          <h2 className="text-4xl font-serif italic">Останні новинки</h2>
      ...
      <section className="bg-[#F3F2EE] rounded-[40px] p-12 text-center space-y-6">
        <h2 className="text-3xl md:text-4xl font-serif">Маєте власний секретний рецепт?</h2>
        <p className="text-gray-600 max-w-lg mx-auto">
          Поділіться ним з нашою спільнотою та надихайте тисячі людей на нові кулінарні звершення!
        </p>
        <Link
          href="/recipes/create"
          className="inline-block bg-gray-950 text-white px-10 py-4 rounded-2xl font-bold hover:scale-105 transition-transform"
        >
          Опублікувати рецепт
        </Link>
      </section>
